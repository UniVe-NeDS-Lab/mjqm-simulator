...
public:
    ...
    std::unique_ptr<Policy> clone() const override {
        return std::make_unique<Orbit>(w, servers, state_buf.size(), sizes, r_max);
    }
    explicit operator std::string() const override {
        return "Orbit(orbit_size=" + std::to_string(r_max) + ", servers=" + std::to_string(servers) + ", classes=" + std::to_string(state_buf.size()) +
